...
const AllStage = () => {
  ...
  const offreList = offres?.offreByStage && Array.isArray(offres.offreByStage)
    ? offres.offreByStage
    : [];
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const action = isActivating
          ? activateOffreStage(offre._id)
          : deactivateOffreStage(offre._id);
        dispatch(action)
          .then(() => {
            dispatch(fetchOffresStage());
          })
      ...
      <style>
        {`
          ...
          .swal-confirm-button:hover {
            ...
            box-shadow: 0 4px 8px rgba(145, 64, 145, 0.3) !important;
